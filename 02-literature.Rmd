...
\begin{equation}
O_3 + hv(\lambda < 1180 nm) -> O + O_2 + energia cinetica 
(\#eq:3)
\end{equation}
...
\begin{equation}
\cdot OH + CO -> \cdot HOCO
(\#eq:4)
\end{equation}


\begin{equation}
\cdot HOCO + O_2 -> HO_2\cdot + CO_2
(\#eq:5)
\end{equation}

\begin{equation}
HO_2\cdot + NO -> \cdot OH + NO_2
(\#eq:6)
\end{equation}

\begin{equation}
NO_2 + hv(\lambda < 400nm) -> NO + O
(\#eq:7)
\end{equation}
...
El es un compuesto que reacciona generando importantes contaminantes como sulfatos y acido sulfurico, importantesen la lluvia aida (deposicion acida). Los combustibles tienen azufre y durante la combustion, el azufre es oxidado generando $SO_2$, diesel tiene mayor cantidad de azufre que gasolina, por lo tanto generando mas $SO_2$. A continuacion una resumen de los mecanismos de generacion de acido sulfurico [@brasseur2017modeling].
